function [x,flag,relres,iter,resvec,H]=idrs(A,b,s,M1,tol,maxit,method,x0,U0,omega)
%
...
%
%   X = IDRS(A,B,S,M1) uses a (left) preconditioner M1. M1 must be N-by-N and invertable.
%
%   X = IDRS(A,B,S,M1,TOL) specifies the tolerance of the method.  If TOL is []
%      then IDRS uses the default, 1e-8. 
%
%   X = IDRS(A,B,S,M1,TOL,MAXIT) specifies the maximum number of iterations.  If
%      MAXIT is [] then IDRS uses the default, min(2*N,1000).
%
%   X = IDRS(A,B,S,M1,TOL,MAXIT,METHOD) specifies a specific method
...
%
%   X = IDRS(A,B,S,M1,TOL,MAXIT,METHOD,X0) specifies a user defined
%      initial guess X0. Default: X0 = 0.
%
%   X = IDRS(A,B,S,M1,TOL,MAXIT,METHOD,X0,U0) specifies a user defined 
%      initial search space U0. This provides a simple way to recycle information.
%      the matrix U0 should be NxS. Default: U0 = [].
%
%   X = IDRS(A,B,S,M1,TOL,MAXIT,IN_S,X0,U0,OMEGA) specifies user values for OMEGA.
...
%
%   [X,FLAG] = IDRS(A,B,S,M1,TOL,MAXIT,IN_S,X0,U0,OMEGA)
...
%
%   [X,FLAG,RELRES] = IDRS(A,B,S,M1,TOL,MAXIT,METHOD,X0,U0,OMEGA)
%      also returns the relative residual norm.
%
%   [X,FLAG,RELRES,ITER] = IDRS(A,B,S,M1,TOL,MAXIT,METHOD,X0,U0,OMEGA)
%      also returns the number of iterations.
%
%   [X,FLAG,RELRES,ITER,RESVEC] = IDRS(A,B,S,M1,TOL,MAXIT,METHOD,X0,U0,OMEGA)
%      also returns a vector with the residual norm at each matrix-vector multiplication.
%
%   [X,FLAG,RELRES,ITER,RESVEC,H] = IDRS(A,B,S,M1,TOL,MAXIT,METHOD,X0,U0,OMEGA)
