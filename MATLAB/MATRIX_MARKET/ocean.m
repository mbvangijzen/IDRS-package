...
%% Menu
   prompt = { 'Example: stommel/sag',...
              ...
              'Inner tolerance:',...
              'Inner iterations:'};
   defaults = { example, grid, variant, s, tol, maxit, in_s, in_tol, in_it};
   lineNo=[1 50];
...
%
% Read the matrices:
   if ( strcmpi(example,'stommel' )  )
      matrix = [pathname, 'STOMMEL/stommel', grid,'.mtx'];
      map = [pathname, 'STOMMEL/stommel', grid,'_map.mtx'];
      rhs = [pathname, 'STOMMEL/stommel', grid,'_b.mtx'];
   else
      matrix = [pathname, 'SAG/sag', grid,'.mtx'];
      map = [pathname, 'SAG/sag', grid,'_map.mtx'];
      rhs = [pathname, 'SAG/sag', grid,'_b.mtx'];
   end
...
%
      ...
      if ( method == 1 || method == 2 )
         [x(:,irhs),flag,relres,iter,resvec] = ...
               idrs(A,b,str2num(s),M,str2num(tol),str2num(maxit),method,x0);
         x0 = x(:,irhs);
      else
         [x(:,irhs),flag,relres,iter,resvec] = ...
             qmridr(A,b,str2num(s),M,str2num(tol),str2num(maxit),...
