function [x, flag, relres, iter, resvec,colfac]=msidrs(A,b,sigma,s,tol,maxit,method,omega );
%
...
%
%   X = MSIDRS(A,B,SHIFT,S,TOL) specifies the tolerance of the method.  If TOL is []
%   then MSIDRS uses the default, 1e-8.
%
%   X = MSIDRS(A,B,SHIFT,S,TOL,MAXIT) specifies the maximum number of iterations.  If
%   MAXIT is [] then MSIDRS uses the default, min(2*N,1000).
%
%   X = MSIDRS(A,B,SHIFT,S,TOL,MAXIT,METHOD) specifies a specific method
...
%
%   X = MSIDRS(A,B,SHIFT,S,TOL,MAXIT,METHOD,OMEGA) defines a sequence of
...
%
%   [X,FLAG] = MSIDRS(A,B,SHIFT,S,TOL,MAXIT,METHOD,OMEGA)
...
%
%   [X,FLAG,RELRES] = MSIDRS(A,B,S,TOL,MAXIT,METHOD,OMEGA) also
%      returns the relative residual norms.
%
%   [X,FLAG,RELRES,ITER] = MSRIDRS(A,B,SHIFT,S,TOL,MAXIT,METHOD,OMEGA) also returns
%      the number of iterations.
%
%   [X,FLAG,RELRES,ITER,RESVEC] = MSIDRS(A,B,SHIFT,S,TOL,MAXIT,METHOD,OMEGA) also returns
%      a vector with, for every shift, the residual norm at each matrix-vector multiplication.
%
%   [X,FLAG,RELRES,ITER,RESVEC,COLFAC] = MSIDRS(A,B,SHIFT,S,TOL,MAXIT,METHOD,OMEGA)
