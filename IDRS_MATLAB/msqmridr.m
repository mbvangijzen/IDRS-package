function [x, flag, relres, iter, resvec]=msqmridr(A,b,sigma,s,tol,maxit,in_s,in_tol,in_it,omega);
%
%MSQMRIDR Multi Shift Induced Dimension Reduction method
%   X = MSQMRIDR(A,B,SIGMA) simultaneously solves the sequence of shifted systems 
%             (A-SHIFT(i) I) X_i = B or (A-SIGMA_i I)X_i = B for X_i.
%
...
%   can can be combined with MSIDRS as inner iterative method. 
%
...
%
%   X = MSQMRIDR(A,B,SHIFT,S,TOL) specifies the tolerance of the method.  If TOL is []
%      then MSQMRIDR uses the default, 1e-8. Note that a cheap upperbound on the residual norms
%      is used as termination criterion.
%
%   X = MSQMRIDR(A,B,SHIFT,S,TOL,MAXIT) specifies the maximum number of iterations.  If
%      MAXIT is [] then MSQMRIDR uses the default, min(2*N,1000).
%
%   X = MSQMRIDR(A,B,SHIFT,S,TOL,MAXIT,IN_S) specifies the parameter S
%      for the inner MSIDRS iterations, default is S=4
%
%   X = MSQMRIDR(A,B,SHIFT,S,TOL,MAXIT,IN_S,IN_TOL) specifies the tollerance
%      for the inner MSIDRS iterations. Default is IN_TOL = 1e-1
%
%   X = MSQMRIDR(A,B,SHIFT,S,TOL,MAXIT,IN_S,IN_TOL,IN_IT) specifies the maximum
