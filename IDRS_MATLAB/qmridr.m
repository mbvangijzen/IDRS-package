function [x, flag, relres, iter, resvec] = ...
   qmridr(A,b,M1,s,tol,maxit, in_s,in_tol,in_it,x0,omega );
%
...
%
%   X = QMRIDR(A,B,M1) uses a (left) preconditioner M1. M1 must be N-by-N and invertable.
%
%   X = QMRIDR(A,B,M1,S) specifies the dimension of the 'shadow space'. If S = [], then
%      QMRIDR uses the default S = 4. Normally, a higher S gives faster convergence,
%      but also makes the method more expensive.
%
%   X = QMRIDR(A,B,M1,S,TOL) specifies the tolerance of the method.  If TOL is []
%      then QMRIDR uses the default, 1e-8. Note that a cheap upperbound on the residual norm
%      is used as termination criterion.
%
%   X = QMRIDR(A,B,M1,S,TOL,MAXIT) specifies the maximum number of iterations.  If
%      MAXIT is [] then QMRIDR uses the default, min(2*N,1000).
%
%   X = QMRIDR(A,B,M1,S,TOL,MAXIT,IN_S) specifies the parameter S 
%      for the inner IDRS iterations, default is S=4
%
%   X = QMRIDR(A,B,M1,S,TOL,MAXIT,IN_S,IN_TOL) specifies the tollerance
%      for the inner IDRS iterations. Default is IN_TOL = 1e-1
%
%   X = QMRIDR(A,B,M1,S,TOL,MAXIT,IN_S,IN_TOL,IN_IT) specifies the maximum
%      of inner IDRS iterations. Default is IN_IT = 0 (no inner iterations)
%
%   X = QMRIDR(A,B,M1,S,TOL,MAXIT,IN_S,IN_TOL,IN_IT,X0) specifies a user defined
%      initial guess X0. Default: X0 = 0.
%
%   X = QMRIDR(A,B,M1,S,TOL,MAXIT,IN_S,IN_TOL,IN_IT,X0,OMEGA) specifies user values for OMEGA.
...
%
%   [X,FLAG] = QMRIDR(A,B,M1,S,TOL,MAXIT,IN_S,IN_TOL,IN_IT,X0,OMEGA)
...
%
%   [X,FLAG,RELRES] = QMRIDR(A,B,M1,S,TOL,MAXIT,IN_S,IN_TOL,IN_IT,X0,OMEGA) also
%      returns an upper bound on the relative residual norm.
%
%   [X,FLAG,RELRES,ITER] =  QMRIDR(A,B,M1,S,TOL,MAXIT,IN_S,IN_TOL,IN_IT,X0,OMEGA) 
%      also returns the number of iterations.
%
%   [X,FLAG,RELRES,ITER,RESVEC] = QMRIDR(A,B,M1,S,TOL,MAXIT,IN_S,IN_TOL,IN_IT,X0,OMEGA) 
