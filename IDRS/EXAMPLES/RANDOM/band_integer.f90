program band_integer

!
! This programme illustrates the use of the idrs-module on an academic integer random band system. 
...
! Output message, problem description:
   if ( my_proc == 1 ) then
      ...
      write(*,'(a)') '========================================================================='
      write(*,'(a,/)') &
         'Randomly generated problem with integer band matrix.'
   end if

! Generate the random matrix in band format:
   allocate(R(nrows,-half_band:half_band),IR(nrows,-half_band:half_band))
...
! Store as standard matrix:
   A = cds_format( nrows, ncols, half_band, iband=IR )
