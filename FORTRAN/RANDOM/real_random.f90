...
!
! This programme illustrates the use of the idrs-module on an academic real random dense system. 
...
! Output message, problem description:
   if ( my_proc == 1 ) then
      ...
      write(*,'(a)') '========================================================================='
      write(*,'(5x,a,i8,/)') &
         'Randomly generated problem with real matrix, size is ', ncols
   end if

! Generate the random matrix in dense format:
   allocate(R(nrows,ncols))
...
! Store as standard matrix:
   A = real_matrix( dense_format( rmat=R ) )
...
! These estimates are based on "approximate" Gershgorin bounds:
   rcenter = 0.5_rp
   rfoci(1) = 0.5_rp-(N-1)*0.5_rp
   rfoci(2) = 0.5_rp+(N-1)*0.5_rp
   rseed    = 0.5_rp-(N-1)*0.5_rp
