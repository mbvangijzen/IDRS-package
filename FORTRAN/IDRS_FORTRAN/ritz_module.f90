!
! The ritz-module contains subrotuines to extract and use spectral 
! information form IDRS.
...
!
! Normalise the Ritzvectors, compute the residual norms
...
!
! Normalise the U0, compute the residual norms
      if ( present(resnrm) ) then
         do j = 1,s
            nrm = norm(U0(:,j))
            U0(:,j) = U0(:,j)/nrm
...
! Optional output arrays:
   complex(kind=cp), optional, allocatable, intent(out) :: U0(:,:)
   ...
   complex(kind=cp), allocatable                        :: Hf(:,:), vec(:,:)
   integer                                              :: n, maxit, converged, i, iter, conv, method
   ...
   if ( present(omega) ) then
      ...
      if ( n_omega > 0 ) then
         ...
         ritzval = QR_eig( Hf(1:n_omega,1:n_omega), converged )
...
!
   ...
   if ( present(U0) ) then
      if ( nritz < s ) stop "Recyle space is too large, reduce s."
      if ( allocated(U0) ) deallocate( U0 )
      allocate( U0(n,s) )

! Compute the Schur vectors of (upper part of) H:
      allocate( vec(nritz,s) )
      vec = SI_power( Hf(:,1:nritz), s )

      allocate( U(size(b),s), R(s,s) )
      if ( present(M) ) then
         U = compute_U0( A, Hf(1:nritz+1,1:nritz), vec, b, M, resnrm )
         do i = 1, s
            U(:,i) = U(:,i)/M
         end do
      else
         U = compute_U0( A, Hf(1:nritz+1,1:nritz), vec, b )
      end if
...
! Optional output arrays:
   real(kind=rp), optional, allocatable, intent(out)    :: U0(:,:)
   ...
   integer                                              :: n, converged, i, iter, conv, method 
...
! Compute the Schur vectors of (upper part of) H:
      allocate( vec(nritz,s) )
      vec = SI_power( Hf(1:nritz,1:nritz), s )
