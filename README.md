...
Often sequences of shifted systems of the form (K+ sigma C +sigma^2 M) x=b, have to be solved for multiple shifts sigma. For this type of problems, special "multishift" IDR methods are included in the package that can solve a sequence of shifted problems almost at the same cost as solving one linear system.
...
## Which preconditioners are included in IDRS?
IDRS comes with a set of preconditioners that has the unique feature that all preconditioners can be applied to the multishift problem. These include:
- Polynomial preconditioners (Chebyshev, Neumann,...);
- (spectral) Deflation preconditioners;
- Shift-and-Invert preconditioners.

These preconditioners can be combined with eachother, and they can of course also be applied to standard linear systems. User defined preconditioners are easy to add. 
## Are interfaces for standard matrix formats supplied?
Yes, dense matrices in DENSE format, band matrices in Compressed Diagonal Storage (CDS) format, and sparse matrices in Compressed Row Storage (CRS) or Coordinate (COO) format can be simply converted to the matrix type that is used in IDRS by one simple subroutine call. After that the complete functionality of IDRS (solvers, preconditioners, parallelisation) is available.
## With what kind of examples does IDRS come?
IDRS comes with a wide range of test problems, in dense, COO, CRS format, and with examples with user defined formats. Test problems range from academic (for example random and toeplitz matrices) to test problems from applications (acoustics, pagerank, ocean circulation, topology optimisation). They include both real and complex examples, and single linear systems, sequences of linear systems and multishift problems.
